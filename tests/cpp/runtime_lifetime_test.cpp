...
// A host that drives the runtime from its own pump gets its timers fired, which is what waiting is.
//
// Under wasm the wait was an emscripten_sleep, which unwinds the whole wasm stack and rewinds it from a
// callback of its own — and a host pumping from its own loop calls back in while that stack is unwound,
// which killed the module outright. Waiting is a timer on the loop on every target now, and a timer on
// the loop is what a poll drains.
TEST(RuntimeLifetime, APumpedRuntimeFiresWhatItWasToldToWaitFor)
{
    varn_runtime* rt = varn_runtime_new();
    ASSERT_NE(rt, nullptr);

    ASSERT_EQ(varn_runtime_load_string(rt, kSleepingChunk, "=pumped"), 0);
    ...
    while (!ran && std::chrono::steady_clock::now() < deadline)
    {
        ...
        ran = varn_runtime_run_string(rt, "assert(ran == 1)", "=check") == 0;
    }
