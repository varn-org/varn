...
`http.client.stream` follows the same way, and a redirect's own body is swallowed rather than handed to `onChunk`.

Under wasm the browser follows a redirect itself and reports only where it ended up, so `redirect` there must be `"follow"`. Asking for `"manual"` or `"error"` is refused rather than answered with the final response as though it were the first.
