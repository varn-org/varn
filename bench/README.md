...
- **Varn** — `http.createServer`, vdo over libmysqlclient and the redis client over a shared
...
connections through a generic [`pool`](https://github.com/varn-org/components/blob/main/pool) — the model `mysql2` and `aiomysql` use.
Redis uses a single **multiplexed connection** ([`pipeline = true`](https://github.com/varn-org/components/blob/main/redis)) that
