...
int AsyncModule::luaSleep(lua_State* L)
{
    ...
    //
    // Under wasm the wait used to be an emscripten_sleep, which unwinds the whole wasm stack and rewinds
    // it from a callback of its own. A host that drives the runtime from its own pump — which is what a
    // user interface does — calls back in while that stack is still unwound, and the rewind then reads a
    // stack nothing holds any more: the module dies with "memory access out of bounds" and every call
    // after it dies the same way. A page dismissing an overlay hit it every time, since a thing leaving
    // the screen is held for as long as it takes to leave.
