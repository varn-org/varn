...
async.run(function()
    ...
    -- The redirect helper answers the status it was given, which is what a client asking to see one gets.
    local pointed = http.client.requestRaw({
        url = base .. "/go",
        redirect = "manual",
        timeoutSeconds = 10,
    }):await()

    assert(pointed.status == 302, "redirect helper failed")

    -- A client that was not told otherwise follows it, the way every other client does.
    assert(statusBody(get("/go")) == 200, "a redirect is followed unless the caller says not to")
    ...
    -- Together jwtAuth and requireRole admit the admin token and deny a missing token.
    ...
    -- The requireAuth middleware admits any authenticated user and denies a request with no token.
