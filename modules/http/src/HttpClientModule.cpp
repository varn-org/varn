...
public:
    ...
    static void readClientOptions(lua_State* L, std::string& method, std::map<std::string, std::string>& headers, std::string& body, varn::http::client::ClientRequestOptions& options)
    {
        ...
        readRedirectOptions(L, options);
    }

    /// Reads what a caller wants done about a response that points somewhere else.
    ///
    /// The names are the ones the fetch standard settled on, since that is what most clients took.
    static void readRedirectOptions(lua_State* L, varn::http::client::ClientRequestOptions& options)
    {
        ...
        if (lua_isstring(L, -1))
        {
            ...
            if (wanted == "follow")
            {
                options.redirects = varn::http::client::RedirectPolicy::Follow;
            }
            else if (wanted == "manual")
            {
                options.redirects = varn::http::client::RedirectPolicy::Manual;
            }
            else if (wanted == "error")
            {
                options.redirects = varn::http::client::RedirectPolicy::Error;
            }
            else
            {
                lua_pop(L, 1);
                luaL_error(L, "redirect must be follow, manual or error, got %s", wanted.c_str());
            }
        }
        lua_pop(L, 1);
        ...
        if (lua_isinteger(L, -1))
        {
            ...
            {
                lua_pop(L, 1);
                luaL_error(L, "maxRedirects cannot be negative");
            }

            options.maxRedirects = static_cast<int>(value);
        }
        lua_pop(L, 1);
    }
};
...
int HttpClientModule::luaClientRequest(lua_State* L)
{
    ...
    // A browser follows a redirect itself and hands over only where it ended up, so a caller asking to
    // see one is told it cannot be, rather than being handed the final answer as if it were the first.
    if (options.redirects != varn::http::client::RedirectPolicy::Follow)
    {
        return luaL_error(L, "a browser follows redirects itself, so redirect must be follow here");
    }
    ...
    rt.ioPool().post([promise, methodStr, urlStr, headers, body, options]
    {
        try
        {
            auto resp = varn::http::client::HttpRedirects::follow(methodStr, urlStr, headers, body, options);
...
int HttpClientModule::luaClientStream(lua_State* L)
{
    ...
    rt.ioPool().post([rtp, promise, methodStr, urlStr, headers, body, options, onChunkRef, onResponseRef]
    {
        ...
        try
        {
            ...
            varn::http::client::HttpRedirects::followStream(methodStr, urlStr, headers, body, options, onResponse, onChunk);
            releaseRefs();
            promise->resolve("ok");
        }
        catch (const std::exception& ex)
